...
function plotQuestionableBehaviours()
% Define variables %
    % Array of all behaviours we expect to see, in ordered number
    behaviours = [
        "Not Questionable", "Questionable Starfish", "Questionable Rudder"];
    ...
    analyses = dir([filePath '/*_questionableBehaviour.csv']);
    ...
    for analysis = 1:length(analyses)
        ...
        experimentName = strsplit(analyses{analysis}, '_questionableBehaviour.csv');
        ...
        behaviourAnalysis = behaviourAnalysis(:, 2:6);
        uniqueBehaviours = unique(behaviourAnalysis);
        % Get the name of the analysis to make a video from
        analysisCSV = strrep(analyses{analysis}, '_questionableBehaviour', '');
        ...
        if useWholeVid == true
            ...
            outputName = [outputPath '/' experimentName '_questionableBehaviours.avi'];
...
function generateBehaviourVid(behaviourAnalysis, behaviour, dlcClean, outputName, labelChoices, videoResolution, behaviourList)
    ...
    for frame = 1:size(dlcClean, 1)
        ...
        if behaviourAnalysis(frame, 1) == behaviour || behaviour == true
            behaviourName = convertStringsToChars(behaviourList(behaviourAnalysis(frame, 1)));
            hold on
            % Plot wing labels
            if labelChoices{1} == 'y'
            ...
            if showRudder == 0
                xlabel(['Front Leg Max: ' num2str(behaviourAnalysis(frame, 2)) ' Front leg Av.: ' num2str(behaviourAnalysis(frame, 4))])
            else
                xlabel(['Distal Deviation Diff: ' num2str(behaviourAnalysis(frame, 3)) 'Distal Deviation Av.: ' num2str(behaviourAnalysis(frame, 5))])
            end
            hold off
            drawnow
